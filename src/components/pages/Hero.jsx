...
import { Database, Settings, Download, Eye, Mail, CodeXml, Server, Cpu } from 'lucide-react';
import { IoLogoJavascript, IoLogoNodejs, IoLogoHtml5,IoMdImages, IoLogoCss3, IoIosChatboxes  } from "react-icons/io";
import { SiExpress, SiPostman ,SiSocketdotio, SiTypescript, SiGraphql, SiDocker, SiPostgresql } from "react-icons/si";
import * as THREE from 'three';
import { OrbitControls } from 'three/examples/jsm/controls/OrbitControls';
import { RiTailwindCssFill } from "react-icons/ri";
import { FaReact, FaGithub, FaNpm, FaAws,FaSwatchbook, FaShoppingCart, FaCar } from "react-icons/fa";
...
const HeroSection = () => {
  ...
  const techStack = [
    // Frontend 
    { name: "HTML5", icon: <IoLogoHtml5 className="w-5 h-5" />, color: "text-orange-500", category: "frontend" },
    { name: "CSS3", icon: <IoLogoCss3 className="w-5 h-5" />, color: "text-blue-500", category: "frontend" },
    { name: "JavaScript", icon: <IoLogoJavascript className="w-5 h-5" />, color: "text-yellow-400", category: "frontend" },
    { name: "Tailwind", icon: <RiTailwindCssFill className="w-5 h-5" />, color: "text-cyan-400", category: "frontend" },
    { name: "React", icon: <FaReact className="w-5 h-5" />, color: "text-blue-400", category: "frontend" },
    { name: "TypeScript", icon: <SiTypescript className="w-5 h-5" />, color: "text-blue-600", category: "frontend" },

    // Backend
    { name: "Node.js", icon: <IoLogoNodejs className="w-5 h-5" />, color: "text-green-500", category: "backend" },
    { name: "Express", icon: <SiExpress className="w-5 h-5" />, color: "text-gray-300", category: "backend" },
    { name: "Socket IO", icon: <SiSocketdotio className="w-5 h-5" />, color: "text-gray-300", category: "backend" },
    { name: "REST API", icon: <CiRoute className="w-5 h-5" />, color: "text-gray-300", category: "backend" },
    { name: "GraphQL", icon: <SiGraphql className="w-5 h-5" />, color: "text-pink-600", category: "backend" },
    { name: "MongoDB", icon: <Database className="w-5 h-5" />, color: "text-green-400", category: "backend" },
    

    // Dev Tools
    { name: "Git", icon: <FaGithub className="w-5 h-5" />, color: "text-white", category: "tools" },
    { name: "Docker", icon: <SiDocker className="w-5 h-5" />, color: "text-blue-400", category: "tools" },
    { name: "AWS", icon: <FaAws className="w-5 h-5" />, color: "text-yellow-400", category: "tools" },
    { name: "npm", icon: <FaNpm className="w-5 h-5" />, color: "text-red-500", category: "tools" },
    { name: "VS Code", icon: <VscVscodeInsiders className="w-5 h-5" />, color: "text-sky-500", category: "tools" },
    { name: "Postman", icon: <SiPostman className="w-5 h-5" />, color: "text-orange-500", category: "tools" },

    // Projects
    { name: "E-Commerce", icon: <FaShoppingCart className="w-5 h-5" />, color: "text-pink-500", category: "project" },
    { name: "LMS", icon: <FaSwatchbook  className="w-5 h-5" />, color: "text-cyan-500", category: "project" },
    { name: "Chat App", icon: <IoIosChatboxes   className="w-5 h-5" />, color: "text-cyan-600", category: "project" },
    { name: "Photo Storage", icon: <IoMdImages  className="w-5 h-5" />, color: "text-indigo-700", category: "project" },
    { name: "Hi-Lift", icon: <GiLift className="w-5 h-5" />, color: "text-pink-100", category: "project" },
    { name: "Vehicle Renting", icon: <FaCar  className="w-5 h-5" />, color: "text-blue-600", category: "project" },
  ];
  ...
  const createBackground = () => {
    ...
    switch (bgMode) {
      ...
      case 2: // 3D Grid
        ...
        const gridHelper = new THREE.GridHelper(gridSize, gridDivisions, 0x3a86ff, 0x3a0ca3);
      ...
      <div className="relative z-10 w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="max-w-2xl">
          ...
          >
            ...
            <motion.h1
              className="text-4xl font-bold leading-tight sm:text-5xl text-white"
            >
            ...
            <motion.h2
              className="text-3xl font-bold mt-4 mb-6 text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500"
            >
            ...
            >
              I craft scalable, performant web applications with cutting-edge technologies. 
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 mb-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {techStack.filter(t => t.category === 'frontend').map((tech, i) => (
                    <motion.div
                      ...
                      className={`flex items-center gap-2 px-3 py-1 rounded-full ${tech.color} ${hoveredTech === 'frontend' ? 'bg-white/10' : 'bg-white/5'}`}
                    >
                      {tech.icon}
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {techStack.filter(t => t.category === 'backend').map((tech, i) => (
                    <motion.div
                      key={i}
                      whileHover={{ y: -3, scale: 1.05 }}
                      className={`flex items-center gap-2 px-3 py-1 rounded-full ${tech.color} ${hoveredTech === 'backend' ? 'bg-white/10' : 'bg-white/5'}`}
                    >
                      {tech.icon}
                      <span className="text-xs font-medium text-white">{tech.name}</span>
                    </motion.div>
                ...
                <div className="flex flex-wrap gap-2">
                  {techStack.filter(t => t.category === 'tools').map((tech, i) => (
                    <motion.div
                      key={i}
                      whileHover={{ y: -3, scale: 1.05 }}
                      className={`flex items-center gap-2 px-3 py-1 rounded-full ${tech.color} ${hoveredTech === 'tools' ? 'bg-white/10' : 'bg-white/5'}`}
                    >
                      {tech.icon}
                      <span className="text-xs font-medium text-white">{tech.name}</span>
                    </motion.div>
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {techStack.filter(t => t.category === 'project').map((tech, i) => (
                    <motion.div
                      key={i}
                      whileHover={{ y: -3, scale: 1.05 }}
                      className={`flex items-center gap-2 px-3 py-1 rounded-full ${tech.color} ${hoveredTech === 'fullstack' ? 'bg-white/10' : 'bg-white/5'}`}
                    >
                      {tech.icon}
                      <span className="text-xs font-medium text-white">{tech.name}</span>
                    </motion.div>
            ...
            <div className="p-4 rounded-lg bg-white/5 backdrop-blur-sm border border-white/10">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h4 className="font-medium text-purple-400">Frontend Mastery</h4>
                </div>
                <p className="text-sm text-white/70">
                  Interactive UIs, Responsive design, State management, and Performance optimization
                </p>
              </div>
              <div className="p-4 rounded-lg bg-white/5 backdrop-blur-sm border border-white/10">
                <div className="flex items-center gap-3 mb-2">
                  <Server className="w-5 h-5 text-blue-400" />
                  <h4 className="font-medium text-blue-400">Backend Expertise</h4>
                </div>
                <p className="text-sm text-white/70">
                  Scalable APIs, Database design, Authentication, and Cloud deployment
                </p>
              </div>
